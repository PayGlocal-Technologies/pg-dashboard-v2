...
/** pg-dashboard's (v1) origin per environment — mirrors `dashboardOrigin()`
 *  in `refer-and-earn/constants.ts` (UAT is on pygcl.com; dev/test/prod stay
 *  on payglocal.in). Kept local rather than imported: that helper is private
 *  to the referral feature. */
 ...
 * dead end dressed up as one: it's a straight navigation to pg-dashboard,
 * using the same per-environment origin logic as `refer-and-earn`'s own
 * `dashboardOrigin()` (kept local here rather than imported, since that one
 * is private to the referral feature).
