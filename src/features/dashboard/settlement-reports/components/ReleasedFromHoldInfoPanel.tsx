  ...
  return (
    <Card className="sticky top-4 gap-5 p-5">
      ...
      <div className="flex flex-col gap-2">
        ...
        <p className="text-sm font-semibold text-foreground">What does &quot;Released from hold&quot; mean?</p>
        <p className="text-xs leading-relaxed text-muted-foreground">
          {count} payment{count === 1 ? " in this settlement was" : "s in this settlement were"} flagged during risk
          or compliance review and temporarily excluded from settlement while under review. Once cleared, they were
          included in the next available settlement cycle, this one.
        </p>
      </div>
      ...
      <div className="flex flex-col gap-2">
        <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Released payments</p>
          ...
          {payments.map((p) => (
            <div key={p.id} className="flex items-center justify-between gap-3 px-3 py-2.5">
              <div className="min-w-0">
                ...
                <p className="truncate text-[11px] text-muted-foreground">{p.releasedFromHold?.reason}</p>
              </div>
              <Icon name="check-circle" size={14} className="shrink-0 text-emerald-500" aria-hidden />
            </div>
          ))}
        </div>
      </div>
