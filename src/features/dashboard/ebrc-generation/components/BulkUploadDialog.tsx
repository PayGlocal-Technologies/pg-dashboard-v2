...
export function BulkUploadDialog({
  ...
  const handleUpload = () => {
    ...
    uploadWorkbook(file, (uploadResult) => {
      ...
      if (errors.length > 0) {
        // Rows the parser rejected stay in the dialog, where they can be read
        // against the file and fixed — nothing was submitted.
        setResult(uploadResult);
        return;
      }
      // Accepted: the workbook is with DGFT now, so this ends at the same
      // "request received" pop-up the wizard's Confirm does rather than a
      // toast — see EbrcRequestReceivedOverlay's own note on why both flows
      // share it.
      close();
  ...
  return (
    <>
      ...
      <EbrcRequestReceivedOverlay open={receivedOpen} onClose={() => setReceivedOpen(false)} />
    </>
  );
}
