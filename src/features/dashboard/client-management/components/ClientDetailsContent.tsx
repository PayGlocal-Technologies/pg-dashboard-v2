...
/**
 * Label left, value right — the arrangement the Client Details reference uses,
 * rather than the Transaction Details column's label-above-value stack, since
 * these values are short enough to sit opposite their labels. `items-start`
 * plus a shrink-0 label lets a long value (an email) wrap onto a second line
 * inside its own half rather than pushing the label out of the card.
 */
...
export function ClientDetailsContent({
  ...
  if (layout === "drawer") {
    // Single column, in document order. The drawer is deliberately the lighter
    // view: the invoice metrics and the transactions list don't render here at
    // all, they're what Expand reveals — the same relationship the transaction
    // drawer has with its own Payment/Sender Details.
    return (
  ...
  // 1/3 + 2/3 grid. The main column is first in the DOM and explicitly placed
  // in column 2 from lg up, so no order utilities are needed: below lg the
  // grid collapses to one column and the natural document order is already the
  // required mobile order (identity → metrics → transactions → contact →
  // account). items-start keeps each column sized to its own content rather
  // than stretching to match the taller one.
  return (
    <div className="grid gap-x-10 gap-y-9 lg:grid-cols-[1fr_2fr] lg:items-start">
      <div className="space-y-9 lg:col-start-2 lg:row-start-1">
        <ClientIdentitySummary client={client} />
      ...
      {/* Supporting detail, deliberately secondary to the column beside it:
          narrower, and carrying only the two reference modules. */}
      <div className="space-y-9 lg:col-start-1 lg:row-start-1">
        <ClientContactSection client={client} />
        <ClientAccountSection client={client} />
      </div>
    </div>
