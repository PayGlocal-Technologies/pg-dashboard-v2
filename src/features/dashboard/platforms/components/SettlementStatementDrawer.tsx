...
import { useAmzAccountStatement } from "@/features/dashboard/multi-currency/hooks";
...
export function SettlementStatementDrawer({
  ...
  // Prefilled from the account but editable, and stored as an *override* rather
  // than seeded state: seeded state would keep the first account's holder name
  // after the merchant switches currency, and re-seeding it from an effect is
  // the setState-in-effect CLAUDE.md rules out. Null means "not edited", so the
  // account's own value shows through and follows the selection.
      ...
      <DrawerContent className="w-full sm:w-[32rem] sm:max-w-[92vw] [&>button:last-child]:hidden">
            ...
            <>
              ...
              <Field>
                ...
                <Input
                  ...
                  value={legalNameOverride ?? account.accountHolderName}
                  onChange={(e) => setLegalNameOverride(e.target.value)}
                  placeholder="Enter Seller Legal Name"
                />
              </Field>

              {/* DBA name and Country are collected for the merchant's own
                  reference — the statement endpoint takes neither, so they stay
                  uncontrolled rather than pretending to be part of the request. */}
              <Field>
        ...
        <DrawerFooter>
          {/* Generation is asynchronous — the request returns a timestamp and the
              ...
              it is at. The two fields the endpoint needs and cannot derive are
          ...
          <Button
            ...
            onClick={() => {
              ...
              void requestStatement({
                ...
                merchantName: legalNameOverride ?? account.accountHolderName,
