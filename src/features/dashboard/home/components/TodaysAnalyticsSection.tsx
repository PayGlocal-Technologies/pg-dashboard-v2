...
function payModeDonutSegmentPath(
  ...
  const xo0 = cx + rOuter * Math.cos(a0),
    yo0 = cy + rOuter * Math.sin(a0);
  const xo1 = cx + rOuter * Math.cos(a1),
    yo1 = cy + rOuter * Math.sin(a1);
...
function VolumeAnalyticsTooltip({
  ...
  return (
    <div
      className={cn(
        "rounded-2xl border border-border bg-card px-4 py-3.5 text-xs antialiased shadow-[0_10px_40px_-14px_rgba(15,23,42,0.2)] dark:shadow-[0_12px_40px_-12px_rgba(0,0,0,0.55)]",
        showPayModeBlock ? "max-w-[min(100vw-1.5rem,22.5rem)]" : "max-w-[min(100vw-1.5rem,17.5rem)]"
      )}
    >
      ...
      {showPayModeBlock && mixT && (
        <>
          <div
            className="my-3 border-t border-zinc-200/80 dark:border-zinc-700/70"
            role="presentation"
          />
          <div className="grid gap-4 sm:grid-cols-[minmax(0,1fr)_minmax(0,1.05fr)] sm:items-stretch sm:gap-0">
            <div className="flex min-w-0 flex-col gap-2.5 sm:pr-4">
              <div>
                <p className="text-[10px] font-bold uppercase tracking-[0.12em] text-foreground/80">
                  Payment mode
                </p>
...
export function TodaysAnalyticsSection({ isLoading }: { isLoading?: boolean }) {
  ...
  const chartRows = useMemo(
    () =>
      todaysAnalytics.hourly.map((row) => ({
        label: row.label,
  ...
  if (isLoading) {
    return (
      <section
        className="grid gap-2 lg:grid-cols-12 lg:items-stretch lg:gap-3"
        aria-label="Today's analytics"
      >
          ...
          <AnimatePresence mode="wait">
            ...
            >
              ...
              <div className="mt-3 flex flex-wrap items-baseline gap-x-2 gap-y-1">
                ...
                {!isPaymentsTab && (
                  <span className="text-[13px] font-normal leading-none text-muted-foreground">
                    INR
                  </span>
                )}
              </div>
            ...
            >
              {TABS.map((t) => (
                <Button
                  ...
                  variant="ghost"
                  onClick={() => setTab(t.id)}
                  className="relative h-auto min-h-0 gap-0 rounded-md border-0 bg-transparent px-2.5 py-1.5 text-left text-xs font-medium transition-colors hover:bg-transparent sm:px-3 sm:text-[13px]"
                >
                  {tab === t.id && (
                    <motion.span
          ...
          <div className="mt-3 space-y-1.5">
            ...
            <div className="flex items-center justify-between gap-2 rounded-lg border border-border bg-muted/30 p-2.5 dark:bg-muted/15">
              <div className="min-w-0 flex-1">
                ...
                <p className="mt-1 text-[11px] font-normal leading-snug text-muted-foreground">
                  {attention.disputes.openCount} open{" "}
                  {attention.disputes.openCount === 1 ? "case" : "cases"}
                </p>
              </div>
              <Button
                variant="outline"
