...
export const CREATE_INVOICE_GUIDE_KEY = "create-invoice-v1";

/**
 * The editor's walkthrough, in the order the page is filled in.
 *
 * Six steps because this screen replaced a six-step wizard: everything is
 * visible at once now, which is faster once you know it and harder the first
 * time, since nothing tells you where to start. A DQA pass called that out —
 * "no guide present when it's a completely new UI and workflow" — and this is
 * the answer. `target` values match the `data-guide` attributes on each section.
 *
...
export const CREATE_INVOICE_GUIDE_STEPS: GuideStep[] = [
  ...
  },
  {
    ...
    description:
      "Each line has a name, rate and quantity. Set the invoice currency here too, and add a discount or tax below the items.",
    side: "top",
    align: "start",
  },
  {
  ...
  },
  {
    ...
    description:
      "Your draft saves itself as you work. Generating finalises the invoice, renders the PDF and lets you email it to your client.",
    side: "bottom",
    align: "end",
  },
];
