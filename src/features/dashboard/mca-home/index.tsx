...
export function McaDashboardFeature() {
  ...
  const displayName = [firstName, lastName].filter(Boolean).join(" ") || profile?.username || "there";
  ...
  function handleInvoice() {
    toast.message("Invoice", { description: "This action isn't wired up yet." });
  }
      ...
      <div className="flex flex-col justify-between gap-2 sm:flex-row sm:items-center">
        <div>
          <h1 className="text-[1.35rem] font-bold leading-snug tracking-tight text-foreground">
            ...
            <span className="inline-block origin-bottom-right" style={{ animation: "wave 2.4s ease-in-out infinite" }}>
              👋
            </span>
          </h1>
        ...
        <div className="flex flex-wrap items-center gap-2.5">
          <div className="hidden items-center gap-1.5 text-[11px] text-muted-foreground sm:flex">
            <Icon name="check-circle" className="h-3 w-3 shrink-0 text-muted-foreground" aria-hidden />
