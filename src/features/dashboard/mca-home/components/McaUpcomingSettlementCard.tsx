 ...
 *
 * TODO(design-review): the account line falls back to a placeholder masked
 * number when useSettlementDetails has nothing (no bank account on file in
 * this environment), purely so the line is visible to review right now.
 * Drop this fallback once every environment this renders in has a real
 * account on file — it should never reach a merchant who genuinely has none.
 */
const PLACEHOLDER_MASKED_ACCOUNT = "****4567";
...
export function McaUpcomingSettlementCard() {
  ...
  const maskedAccountNumber = settlement?.maskedAccountNumber ?? PLACEHOLDER_MASKED_ACCOUNT;
      ...
      {!isLoading && upcoming && upcoming.pendingInvoiceCount > 0 ? (
        <>
          ...
          <p className="text-xs text-muted-foreground">
            ...
            invoice{upcoming.pendingInvoiceCount === 1 ? "" : "s"} before 8pm today to receive
