...
import { McaWidgetPickerTile } from "@/features/dashboard/mca-home/components/widgets/McaWidgetPickerTile";

function emptyGrouped(): Record<McaWidgetCategory, typeof MCA_WIDGET_CATALOG> {
  const m = {} as Record<McaWidgetCategory, typeof MCA_WIDGET_CATALOG>;
  for (const c of MCA_CATEGORY_ORDER) m[c] = [];
  return m;
}

function filterCatalogByQuery(query: string): Record<McaWidgetCategory, typeof MCA_WIDGET_CATALOG> {
  ...
  for (const w of MCA_WIDGET_CATALOG) {
    ...
    if (name.includes(q) || cat.includes(q) || w.id.toLowerCase().replace(/-/g, " ").includes(q)) {
      m[w.category].push(w);
    }
  }
  return m;
