 ...
 *
 * TODO(backend): the saved-amount endpoint returns only a current figure, no
 * prior-period comparison field to compute a real trend from. The chip below
 * is a placeholder percentage, not derived from any real prior-period
 * amount — swap it for the real figure the moment the endpoint carries one.
 */
const PLACEHOLDER_TREND_PCT = 12.4;
          ...
          <div className="mt-1.5 flex items-baseline gap-2">
            ...
            <span
              className={cn(
                ...
                PLACEHOLDER_TREND_PCT >= 0
                  ? "text-emerald-600 dark:text-emerald-400"
                  : "text-red-600 dark:text-red-400"
              )}
            >
              <Icon
                name={PLACEHOLDER_TREND_PCT >= 0 ? "trending-up" : "trending-down"}
                size={11}
                aria-hidden
              />
              {PLACEHOLDER_TREND_PCT >= 0 ? "+" : ""}
              {PLACEHOLDER_TREND_PCT}%
            </span>
          </div>
        )}
