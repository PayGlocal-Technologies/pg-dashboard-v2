...
const SIZE = 440;
/** Resolution note: this uses world-atlas's 110m topology, not 50m. The globe
 * displays at 320 CSS px, so 50m's ~99.5k vertices (vs 110m's ~10.6k) were
 * being thrown away by the downscale while still costing a full orthographic
 * re-projection of roughly half of them on every animation frame, which
 * saturated the main thread and made the dashboard's other charts animate
 * choppily. Bump back to countries-50m.json only if the globe ever renders
 * substantially larger than the viewBox it's scaled into. */
...
export function McaGlobeIllustration({ highlights = [] }: McaGlobeIllustrationProps) {
  ...
  /** Gates the rotation loop below, see the IntersectionObserver further
   * down: the loop is by far the most expensive thing on this dashboard and
   * has no reason to run while the globe is scrolled out of view or the tab
  ...
  const highlightByCode = useMemo(
    () => new Map(highlights.map((h) => [h.countryCode, h])),
    [highlights]
  );
  ...
  }, [pathGenerator, projection, prefersReducedMotion, loopActive]);
  ...
  // Runs the rotation loop only while the globe is actually on screen and the
  // tab is foregrounded. The initial paths are rendered by countryPaths above,
  // so a globe that has never been scrolled to still looks correct, it just
  // isn't spinning (and isn't projecting geometry) until it is worth doing.
  useEffect(() => {
    const el = containerRef.current;
    if (!el) return;
    let onScreen = false;
    const sync = () => setLoopActive(onScreen && !document.hidden);
  ...
  useLayoutEffect(() => {
    if (
      activeMarkerX === undefined ||
      activeMarkerY === undefined ||
  ...
  function handlePointerMove(e: ReactPointerEvent<SVGSVGElement>) {
    ...
    phiRef.current = Math.max(
      -MAX_TILT_DEG,
      Math.min(MAX_TILT_DEG, phiRef.current - dy * DRAG_SENSITIVITY)
    );
  }
  ...
  return (
    <div ref={containerRef} className="relative h-full w-full">
      ...
      >
        ...
        {outline && (
          <path d={outline} fill="none" stroke="#c3d0e8" strokeWidth={RIM_STROKE_WIDTH} />
        )}
      </svg>
            ...
            <div className="flex flex-col gap-1.5 text-xs">
              ...
              <div className="flex items-center justify-between gap-3">
                ...
                <span className="font-semibold text-foreground">
                  {activeHighlight.invoiceCountLabel}
                </span>
              </div>
              <div className="flex items-center justify-between gap-3">
