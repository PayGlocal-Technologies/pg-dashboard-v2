...
/**
 * The earliest capture day that settles on `settlementDate`: the inverse of
 * computeSettlementSchedule, found by walking backwards until the T+1 rules
 * land on that date again.
 *
 * Derived rather than fetched — no settlement endpoint carries the capture day.
 * Note the true answer is a RANGE, because a weekend or holiday rolls several
 * capture days into one settlement: a Friday, Saturday and Sunday capture all
 * settle on the Monday. This returns the earliest of them, which is where the
 * non-working-day explanation starts.
 */
export function previousCaptureDay(settlementDate: string, holidays: HolidayInfo[]): string {
  // Guarded rather than trusted: this walks the calendar with Date arithmetic,
  // so a null or malformed key from the API would throw inside a render and
  // take the whole page down with it (it already did once). Handing the input
  // back unchanged degrades to "no pushout detected", which is the right
  // answer for a date we cannot reason about.
  if (!DATE_KEY_PATTERN.test(settlementDate ?? "")) return settlementDate;

  let earliest = addDays(settlementDate, -1);
  for (let back = 1; back <= MAX_CAPTURE_LOOKBACK_DAYS; back++) {
    const candidate = addDays(settlementDate, -back);
    if (computeSettlementSchedule(candidate, holidays).settlementDate === settlementDate) {
      earliest = candidate;
    }
  }
  return earliest;
...
/** Long enough to clear any run of weekend plus consecutive bank holidays. */
const MAX_CAPTURE_LOOKBACK_DAYS = 10;
