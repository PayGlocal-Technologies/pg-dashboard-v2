...
export const totalVolumeChartsByTimeframe: Record<
  TotalVolumeTimeframe,
  TransactionsSparklinePoint[]
> = {
  today: [
    { x: "12 AM", y: 4200 },
    { x: "4 AM", y: 2100 },
...
function deriveMetrics(raw: RawPeriodMetrics): TransactionsMetrics {
  ...
  const successRate =
    totalAttempts === 0 ? 0 : round((raw.transactionCount / totalAttempts) * 100, 1);
  const totalAttemptsPrev = raw.transactionCountPrev + raw.failedCountPrev;
  const successRatePrev =
    totalAttemptsPrev === 0 ? 0 : (raw.transactionCountPrev / totalAttemptsPrev) * 100;

  const averageTicketSize =
    raw.transactionCount === 0 ? 0 : round(raw.totalVolume / raw.transactionCount, 2);
...
export const transactionsTrendChartsByTimeframe: Record<
  TotalVolumeTimeframe,
