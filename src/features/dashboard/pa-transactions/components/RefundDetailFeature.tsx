...
import { getRefundDetailLinkedRows } from "@/features/dashboard/pa-transactions/linkedChildRecords";
...
export function RefundDetailFeature({ transactionId, refundId }: RefundDetailFeatureProps) {
      ...
      <div className="-m-4 min-h-[calc(100vh-57px)] bg-card p-4 md:-m-6 md:p-6">
        <div className="page-enter mx-auto max-w-350 space-y-4">
          <div className="flex flex-col items-center gap-3 rounded-xl border border-border bg-card p-10 text-center">
            ...
            <Button variant="outline" size="sm" onClick={() => router.push("/pa-transactions")}>
              Go back
            </Button>
          </div>
        </div>
  ...
  function goToParent() {
    ...
    router.push(`/pa-transactions/${encodeURIComponent(transactionId)}`);
  }
