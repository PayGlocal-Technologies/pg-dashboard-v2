...
} from "@/features/dashboard/mca-v2/mock-data";
...
export function McaV2Feature() {
  ...
  return (
    <div className="mx-auto max-w-[1400px] page-enter">
      ...
      {/* Summary row: full page width, directly under the page header and
          above the two-column section, so the figures read as the page's own
          headline rather than as part of the receiving-account details.
          items-start lets each card keep its own height — stretching the short
          Outstanding card to the settled card's chart height would leave a
          block of dead space inside it. */}
      <div className="mb-8 grid items-start gap-4 md:grid-cols-2">
        ...
        <Card className="gap-0 p-5">
          ...
          <div className="mt-1 text-sm tabular-nums text-muted-foreground">{settled.valueInr}</div>
          ...
          <div className="mt-6 h-36 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={settled.trend} margin={{ top: 8, right: 12, left: 0, bottom: 0 }}>
                <defs>
      ...
      {/* Both columns are titled modules of the same shape: a title block, then
          ...
          Row 1 carries both column titles, so it sizes to the taller of the
          two and the region Card and the Account Details card below them start
          on exactly the same line, however many lines each title's supporting
          copy wraps to. Two column wrappers could only match those edges by
          hard-coding a header height. gap-x-10 is the gutter, gap-y-5 the 20px
          title → content step. */}
