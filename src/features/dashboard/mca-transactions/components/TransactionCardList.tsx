  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2">
        ...
        {/* View details stays here, icon-only, opening the same details
            drawer the card's own click does — it names that action rather
            than being a second, different one. Still calls
            e.stopPropagation() so the card's handler doesn't also fire.
            Upload invoice moves to its own bottom-right row below instead of
            this slot: unlike View, it's a labelled action, and a label here
            would fight the amount for room on this row. */}
        {!showUpload && (
