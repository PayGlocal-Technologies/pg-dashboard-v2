 ...
 * the KPI below is that one real value alone rather than a fabricated
 * second currency.
 */
export function OutstandingAmountCard({ className }: { className?: string }) {
  ...
  const fundsOnHold = toMetricNumber(overview?.fundsOnHold?.value);
    ...
    <Card size="sm" className={cn("w-full", className)}>
      ...
      <CardContent className="flex flex-1 flex-col">
        ...
        {/* Funds on hold is a strict subset of what is outstanding, so it
            reads as supporting detail under the KPI rather than a second
            figure competing with it. Hidden entirely when nothing is held,
            instead of showing a zero. */}
        {!isLoading && fundsOnHold > 0 && (
          <p className="mt-1 text-sm tabular-nums text-muted-foreground">
            {formatCurrency(fundsOnHold, "INR", "en-IN")} on hold
          </p>
        )}
